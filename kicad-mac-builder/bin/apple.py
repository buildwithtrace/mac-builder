...
def get_kicad_paths_for_signing(dotapp_path):
    ...
    # Skip python.o - object files (.o) cannot be signed with codesign
    # They are intermediate build artifacts and should not be in the app bundle anyway
    ...
    to_sign.append(os.path.join(dotapp_path, "Contents/MacOS/trace-cli"))
    to_sign.append(os.path.join(dotapp_path, "Contents/MacOS/trace"))
...
def sign_file(path, key_label, hardened_runtime, secure_timestamp, entitlements_path=None):
    # Skip signing if file doesn't exist
    if not os.path.exists(path):
        logging.warning("File does not exist, skipping: {}".format(path))
        return
    ...
    try:
        ...
        # For object files (.o), signing might fail - log and continue
        if path.endswith('.o'):
            logging.warning("Failed to sign object file {} (this may be expected): {}".format(path, e))
            return
        raise
