...
setup_dmg()
{
    ...
    diskutil unmount /Volumes/KiCad || true  # Unmount old volume name if it exists
    DEVICE=$(hdiutil attach "${TEMPLATE}" -noautoopen -nobrowse -mountpoint "${MOUNTPOINT}" | awk '/Apple_HFS/ {print $1}')
    # Rename the volume to match MOUNT_NAME (in case template has different name)
    VOLUME_NAME=$(diskutil info "${DEVICE}" | grep "Volume Name" | awk -F': ' '{print $2}' | xargs)
    if [ "${VOLUME_NAME}" != "${MOUNT_NAME}" ]; then
        diskutil rename "${DEVICE}" "${MOUNT_NAME}" || diskutil rename "${MOUNTPOINT}" "${MOUNT_NAME}"
    fi
    # Clean up any old folders from template (but preserve Trace if it exists to keep .DS_Store positions)
    # Only remove KiCad folder, keep Trace folder structure to preserve icon positions
    rm -rf "${MOUNTPOINT}"/KiCad 2>/dev/null || true
    # Clear Trace folder contents but keep the folder itself to preserve .DS_Store entry
    if [ -d "${MOUNTPOINT}"/Trace ]; then
        rm -rf "${MOUNTPOINT}"/Trace/* "${MOUNTPOINT}"/Trace/.* 2>/dev/null || true
    fi
    # Remove old volume icon and set new Trace volume icon
    rm -f "${MOUNTPOINT}"/.VolumeIcon.icns 2>/dev/null || true
    if [ -f "${PACKAGING_DIR}"/VolumeIcon.icns ]; then
        cp "${PACKAGING_DIR}"/VolumeIcon.icns "${MOUNTPOINT}"/.VolumeIcon.icns
        SetFile -a C "${MOUNTPOINT}"/.VolumeIcon.icns
    fi
...
fixup_and_cleanup()
{
    # Remove any old background images (but preserve .DS_Store which contains window view settings)
    rm -rf "${MOUNTPOINT}"/.background "${MOUNTPOINT}"/.background.png "${MOUNTPOINT}"/background.png 2>/dev/null || true
    # Copy Trace logo background to the same location the template expects (.background.png at root)
    # The template's .DS_Store already references this location
    cp "${PACKAGING_DIR}"/background.png "${MOUNTPOINT}"/.background.png
...
echo "TRACE_VERSION: ${TRACE_VERSION}"
...
case "${PACKAGE_TYPE}" in
    ...
    unified)
        ...
        if [ -n "$TRACE_VERSION" ]; then
            DMG_NAME=trace-"${TRACE_VERSION}"-macos-universal.dmg
        elif [ -n "$RELEASE_NAME" ]; then
            DMG_NAME=trace-"${RELEASE_NAME}"-macos-universal.dmg
        else
            DMG_NAME=trace-"${NOW}"-"${KICAD_GIT_REV}"-macos-universal.dmg
        fi
    ;;
    *)
        echo "PACKAGE_TYPE must be either \"nightly\", \"extras\", or \"unified\"."
...
case "${PACKAGE_TYPE}" in
    ...
    unified)
        ...
        rsync -al "${KICAD_INSTALL_DIR}"/* "${MOUNTPOINT}"/Trace/. # IMPORTANT: must preserve symlinks
        if [ -d "${MOUNTPOINT}"/Trace/demos ]; then
            echo "Moving demos"
            mv "${MOUNTPOINT}"/Trace/demos "${MOUNTPOINT}"/
        else
            echo "No demos directory found, skipping"
        fi
    ;;
    *)
